...
class CostConstrainedStatisticTestStateMultipleCost : public mcts::StateInterface<CostConstrainedStatisticTestStateMultipleCost>, 
  public RandomGenerator
{
public:
    CostConstrainedStatisticTestStateMultipleCost(int n_steps, Cost collision_risk1, Cost collision_risk2,
                                     Reward reward_goal1, Reward reward_goal2, bool is_terminal, unsigned int seed = 1000) :
                                     CostConstrainedStatisticTestStateMultipleCost(0, n_steps, collision_risk1, collision_risk2,
    ...
    ~CostConstrainedStatisticTestStateMultipleCost() {};

    std::shared_ptr<CostConstrainedStatisticTestStateMultipleCost> clone() const {
        return std::make_shared<CostConstrainedStatisticTestStateMultipleCost>(*this);
    }
    ...
    std::shared_ptr<CostConstrainedStatisticTestStateMultipleCost> execute(const JointAction& joint_action, std::vector<Reward>& rewards, EgoCosts& ego_cost) const {
        ...
        if(ego_agent_action == 0) {
            rewards[0] = 0;
            ego_cost = {0.0f, 0.0f};
            return std::make_shared<CostConstrainedStatisticTestStateMultipleCost>(0, n_steps_, collision_risk1_, collision_risk2_,
          ...
          if (new_state >= n_steps_) {
            ...
            is_terminal = true;
          } else if (safety_violated) {
            rewards = std::vector<Reward>{0.0f};
            ego_cost = get_safety_violated_cost();
            is_terminal = true;
          } else {
            rewards[0] = 0;
            ego_cost = get_other_cost();
          }
          return std::make_shared<CostConstrainedStatisticTestStateMultipleCost>(new_state, n_steps_, collision_risk1_, collision_risk2_,
    ...
    std::vector<Cost> get_safety_violated_cost() const {
       return {1.0f, 0.0f};
    }
    ...
    {
        std::stringstream ss;
        ss << "CostConstrainedStatisticTestStateMultipleCost (current_state: " << current_state_ << ")";
        return ss.str();
    }
private:
    int current_state_;
