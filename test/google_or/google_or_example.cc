...
void SimpleLpProgram() {
    std::vector<double> action_values{0.05, 0.1, 0.2, 0.3};
    ...
    for (std::size_t action_idx = 0; action_idx <  action_values.size(); ++action_idx) {
    ...
    for (std::size_t lambda_idx = 0; lambda_idx <  lambdas.size(); ++lambda_idx) {
        ...
        for (std::size_t action_idx = 0; action_idx <  action_values.size(); ++action_idx) {
            ct->SetCoefficient(action_weights[action_idx], action_values[action_idx]);
        }
